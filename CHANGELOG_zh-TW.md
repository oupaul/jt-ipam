...
### 變更
- **升級沒辦法自己修好語意搜尋，所以設定頁改成把話講出來，而不是繼續沉默。** 0.5.148 修好了出廠預設、也讓失敗會回報，但對既有站台這些都不會自動生效：資料庫裡存過的嵌入模型優先於新預設、替代模型仍要自己在 LLM Server 上 pull、既有資料的向量要跑過重建索引才會有。升上來的站台語意搜尋依舊沒有結果，畫面上依舊沒有任何說明 —— 跟修之前一樣沉默。

  設定頁現在**載入時就自動驗一次維度**，模型的輸出與資料庫欄位對不上會直接寫出來。另外加上「**重建索引**」按鈕：那個端點一直都在，只是介面上沒有任何入口，於是使用者根本沒有辦法讓語意搜尋真的開始運作。中英安裝指南也各補了一段「從舊版升級的人請特別看」，明列那三件只能手動的事。

  做到全自動既不可能也不該：本專案沒辦法去別人的 LLM Server 上 pull 模型，而使用者自己選過的模型，升版不該悄悄改掉。能做的是讓失敗大聲說話，並且把修復收斂成一次點擊。
...
### 變更
- **VMware ESXi / vCenter 整合終於寫進 README 與專案網頁**。它是 0.5.148 的主打功能，卻在這兩處一個字都沒有 —— README 裡 Proxmox VE 出現 5 次，VMware 0 次。沒有人知道存在的功能，等於不存在。
- **「掃描節奏」改成「掃描頻率」**。節奏不是台灣的說法。
- **各探測的間隔改成「名稱｜數值｜換算」三欄對齊**，不再是六個各佔一整列的欄位。六種探測會把對話框拉成長長一條，要比較兩個間隔還得上下捲。

### 修正
- **路徑追蹤改成一跳一列即時出現**，不再是按下去一分鐘毫無反應。沒有回應的躍點必須等滿逾時才能確定，15 跳要 30～60 秒 —— 那段期間畫面完全沒有動靜，使用者分不出是在跑、卡住、還是壞了。

  這裡有一個不實測就會安靜失效的地方：`tracepath` 的 stdout 接到 pipe 時是**整塊緩衝**的，所有行會等行程結束才一次湧出（實測 6.02 秒時全部到達）。指令改由 `stdbuf -oL` 帶起來之後，才真的是 +0.02、+3.02、+6.03 逐行送達。回應另外帶 `X-Accel-Buffering: no`，因為 nginx 預設會把整個串流憋到結束。少了其中任何一個，串流的程式碼看起來都對，畫面上卻什麼都不會改變。

- **Ping 的封包之間有間隔了，而且兩條路徑一致。** 選「次數 10」按下去立刻就出數字：它確實送了 10 個也收了 10 個，但整批在 53 毫秒內打完。50 毫秒的連發看不到抖動、看不到間歇掉包，對 ICMP 有速率限制的裝置還會回報出不存在的遺失。更嚴重的是兩條路徑量的不是同一件事 —— 能開 ICMP socket 的機器 0.05 秒跑完，退回外部 `ping -c 10` 的約 9 秒，而你拿到哪一種取決於這台機器。現在兩條都是 0.25 秒間隔（10 次約 2.3 秒），已在 prod 實測核對。
- **IP 懸停摘要把兩個英文原始值並排顯示** —— `active` 與 `unknown` —— 看起來像一個自相矛盾的狀態。那其實是兩個不同的欄位：一個是登記的用途，一個是監測實際量到的存活。現在分成兩列各自標示，並沿用全站既有翻譯，`online (scanner)` 會顯示成「上線（scanner）」。另外補了元件測試直接斷言畫面上畫出什麼 —— 這種純顯示的缺陷，型別檢查抓不到。

## [0.5.148] — 2026-08-06

### 新增

- **可接 OpenAI 相容端點的 LLM。** 供應商設定在 Ollama 之外多了 OpenAI 相容模式，可接 ChatGPT、vLLM、LM Studio、OpenRouter 等，以及 Ollama 自己的 `/v1` 相容層。

  **預設仍是 Ollama，要切換得自己動手。** 這個專案的前提是「自架模型、資料留在自己的網路裡」；把網段、主機名稱與拓樸送到外部服務，是使用者要明確做的決定，不是升版就自動改變的行為。設定頁在選了外部供應商時會直接把這件事寫出來，不留給人自行意會。

  兩家的差異是實際存在的，逐項處理而非含糊帶過：對話與嵌入的路徑不同、回應結構不同、`options`（`num_ctx`）是 Ollama 專屬送過去會被拒絕，模型清單走 `/v1/models` 而不是 `/api/tags` —— 最後這項若漏掉，模型下拉會安靜地變成空的，畫面上沒有任何線索說明原因。使用者填的網址已經以 `/v1` 結尾時不會重複補一次。沒設定金鑰就不送，因為本地端點多半不需要，而空的 `Bearer` 會被判成認證失敗。

  金鑰**加密存放**（AES-GCM，使用專屬 AAD），與本專案其他機密一致 —— 雲端服務的付費憑證不該以明文躺在 `system_settings` 裡，讓資料庫備份或一次誤開的 `psql` 就看得到。金鑰不會回傳到瀏覽器，設定頁只顯示「已設定」。

- **VMware ESXi / vCenter 整合（Beta）**。同一套實作**同時涵蓋單機 ESXi 與 vCenter** —— 兩者都是 `/sdk` 上的同一組 VIM API，ContainerView 把清單層次的差異吸收掉了。虛擬機寫進**與 Proxmox 相同的資料表**，所以拓樸、AI 對話與 MCP 的 `list_vms` 一行都不必改。

  **SOAP 是自己送的，沒有用 pyvmomi**。那個 SDK 會繞過 `safe_request` —— 也就是做 SSRF 檢查、每次重導向後重新驗證 URL、套用設定的 TLS 驗證的那一層 —— 而本專案每一個對外整合都走它。唯讀的清單查詢其實只需要五個呼叫，為了省一點功夫而在安全架構上破例並不划算。順帶的好處是不必新增任何套件。

  全程唯讀，不對 ESXi 寫入任何東西。解析刻意容忍缺欄位，因為那些在正常運作下就是會缺的 —— 關機的 VM 沒有 `guest.*`、沒裝 VMware Tools 的回報不出位址、範本沒有 `runtime.host`。分頁的續抓 token 一定會跟完，因為漏掉它會**無聲地**少掉大型環境的後半。

  設定頁的「測試連線」逐步回報，而不是只給成功或失敗：卡在哪一個呼叫才是要知道的事。密碼打錯會顯示 VMware 自己的訊息 —— VMware 把認證失敗包成 SOAP Fault 用 HTTP 500 送回來，不處理的話看起來只是「伺服器錯誤」。

- **虛擬化檢視頁拆成「虛擬化 (Proxmox VE)」與「虛擬化 (VMware)」**，各自只列自己平台的內容。

- **語意搜尋從來沒有真的運作過，而且是每一套安裝都一樣。** 出廠預設的嵌入模型回 4096 維，資料庫欄位是 `vector(768)`，所以每一筆索引寫入都丟例外 —— 然後被一個 `return False` 吞掉。prod 上三張表的 embedding 全是 0 筆。畫面上從來沒有任何訊息說過這件事：整表重建索引回報的是 `{subnets: 0, ip_addresses: 0, devices: 0}`，跟「本來就沒有東西要索引」長得一模一樣。

  改了三處，因為真正的缺陷是那份沉默：重建索引現在會回報**失敗幾筆、原因是什麼**（同一支在 prod 上再跑一次就說出 `failed: 97` 與維度不合的細節）；設定頁多了**檢查維度**按鈕，實際跟模型要一次向量，把「模型回幾維、欄位要幾維」講出來；預設模型改成 768 維的 `granite-embedding:278m`。

  換哪一個是**測出來的，不是看維度數字挑的**。`nomic-embed-text` 同樣是 768 維，但對不同的中文描述回**完全相同的向量** —— 它是英文專用模型，那些中文被吃成同一串未知詞。用它會看起來像修好了，實際上排序是亂的。真正採用的模型是拿現場實際在用的描述驗過、連只差一個字的兩段都分得開，才敢換上去。

### 變更
- **掃描代理的每一種探測都可以設定間隔了**，不再只有重量探測。後端本來就接受全部七種、也各自對最小值做把關；輕量那六種只是沒有欄位可設，所以一直卡在預設值。頁面同時算出實際頻率（「目前每 5 分鐘掃一輪」），因為快迴圈等於最短的輕量間隔 —— 這個耦合關係原本只存在於程式碼裡。
- **i18n 檢查改成單雙引號都掃**。原本只比對 `t("…")`，於是 `t('addresses.os')` 整條被跳過 —— 那個鍵並不存在，畫面上直接顯示原始鍵名。強化之後它立刻被抓出來，而且全專案就這一個。
- **調查視窗的交換器位置改用 `裝置@埠號`**，與 IP 詳細資料頁一致。格式化函式抽成共用，不再各寫一份 —— 兩份實作遲早會有一份忘記跟上。

### 修正
- **整表重建索引會與整合同步互鎖**、整批中止 —— 它把 `ip_addresses` 的每一列放在同一個交易裡，而 `jt-ipam-sync` 每五分鐘就在更新同一批資料。改成分批 commit，衝突視窗從整張表縮到 25 筆。這是在 prod 上第一次真的跑得動重建索引時撞出來的。
...
### 新增
- **調查模式**。在位址上按一個鈕，把散落在各處的線索收進同一個畫面：本身的紀錄、重疊網段下同一位址的其他紀錄、各來源分別回報的主機名稱與 OS、監控涵蓋、ARP 歷史、近期異動，以及（具全域讀取權限者才有的）DNS、NAT、防火牆規則。**矛盾之處會算出來放在最上面** —— 那正是這個功能的重點：各來源的主機名稱對不上、已失聯的 agent 還掛在這個位址、同一個位址上看過好幾個不同的 MAC。

  這週修掉的兩個問題，追查時每次都得在六個畫面之間翻。那個把 macOS 算到 Linux VM 頭上的位址，檔案一拉出來整個故事就在上面 —— 四個來源四個不同的名字，加上一個已失聯的 macOS agent 還掛著。

  事實與推測分開：檔案是查得到的事實，模型的判讀要按了才產生，而且標明是推測。模型不可用時這個功能仍然完整可用 —— 真正省時間的是把線索收在一起，不是那段敘述。AI 對話與 MCP 也能用（`investigate_ip`）。

## [0.5.145] — 2026-08-05

### 新增
- **異常偵測新增三條規則**，全部是算得出來的事實，不是推測：
  - **懸空 DNS** —— A／AAAA 記錄指向 IPAM 裡根本不存在的位址。對外網域時這是**子網域接管**的前置條件：名字還解析得到、位址已經沒人管，誰拿到那個位址就等於拿到那個名字。只看 A／AAAA，因為 CNAME 的值是名字不是位址，拿去比對 IP 必定「找不到」。線上實測 8 筆，其中一筆指向 Docker 的橋接位址。
  - **重疊網段的重複紀錄** —— 同一個位址在互相**包含**的兩個子網路裡各有一筆。兩個單位登記一模一樣的 CIDR 是刻意支援的多租戶用法，**不報**；包含關係則幾乎必定是誤建。要報的理由是：整合同步只會標到其中一筆，另一筆的存活狀態就此凍結 —— 線上就這樣讓一台正常運作的機器顯示離線、可用率 0%。
  - **可疑的變更** —— 單一帳號短時間大量刪除、同一來源反覆登入失敗、以及任何權限／帳號／token 的變更。**沒有操作者的刪除會排除**：那是整合同步刪掉自己的舊資料再重建（實機上一次同步 19 分鐘刪了 967 筆），算進來的話清單第一名永遠是例行作業，真正的人為誤刪反而被埋掉。「非上班時段」刻意不做：它需要可靠的時區與工時設定，而一條會誤報的規則會訓練人忽略整個清單。

### 變更
- **AI 巡檢「依據資料」的主機名稱懸停也會顯示摘要卡**，與 IP 一致。兩者都是查證這筆發現的線索，查起來的成本就該一樣。

## [0.5.144] — 2026-08-05

### 新增
- **裝置頁顯示資安組態評估（SCA）**。Wazuh 會依基準（CIS、廠商自訂）逐項檢查每台主機並給分，jt-ipam 現在把結果存進 agent 並顯示在 Wazuh 卡片上。一台機器跑多個基準時顯示**分數最低**的那一個 —— 挑好看的等於自我安慰。線上實測 35 台有資料，最低的一台 23 分（112 項通過、361 項未通過）。

  這裡刻意**不是**漏洞（CVE）數。Wazuh 在 4.8 已把漏洞端點全數從 manager API 移除 —— 我把這台伺服器實際提供的 150 條路徑逐條列出確認過，沒有任何一條與漏洞相關 —— 唯一剩下的來源是 Wazuh Indexer。接上去要多一組長期保存、且**讀得到整個 SIEM 所有告警**的憑證，換來的只是兩個數字，還要綁在一個未來改版可能更名的內部索引名稱上。這筆交易不划算，所以不提供這個整合；先前寫好的 Indexer 版本在發布前就撤掉了，而不是半推半就地送出去。

### 變更
- **整合遇到不明確的位址時不再用猜的**。重疊網段下 —— 兩個單位都用 192.168.1.0/24 —— 同一個 IP 字串是兩台不同機器。Wazuh 原本用 dict 建對應表（後面的列無聲覆寫前面的）、LibreNMS 取第一筆，所以資料落到哪一筆紀錄取決於資料庫的回傳順序。兩者現在都改成：一個位址對應到多筆就**不對應**，並回報跳過了幾筆。因為**掛錯比沒有更糟** —— 沒有資料您會去查，掛錯了您不會發現；跨單位時那更是資料外洩。在該整合設定「限定子網路範圍」把候選縮回一筆，就會恢復對應。

## [0.5.143] — 2026-08-05

### 修正
- **Wazuh 卡片對從未檢查過的機器顯示「0 / 0」個漏洞**。抓取漏洞的程式打的是 manager API 的 `/vulnerability/*`，那些端點在 **Wazuh 4.8 就已移除**（線上這台是 4.14.5，直接回 404）。例外被吞掉、欄位維持 NULL、畫面把 NULL 顯示成 0。對一台從沒檢查過的機器宣告「沒有漏洞」，比什麼都不顯示更糟。

## [0.5.142] — 2026-08-05

### 修正
- **AI 巡檢的發現逐次累加，而不是取代**。四次排程執行下來累積了 62 筆未處理，大半是同樣那幾件事換句話說。指紋是「分類＋引用到的 IP 集合」，而模型每次分組的方式都不同 —— `{.97,.46,.129} ＋ {.54}` 隔天變成 `{.54,.129,.46} ＋ {.97}`，指紋就不一樣了。巡檢是「現在有什麼問題」的快照，不是逐次累加的流水帳，所以每次執行改成**對齊**未處理清單：仍然存在的保留原本的發現時間、已經不在的移除、已忽略的維持原樣當作抑制的依據（不再每輪補存一份複本）。
- **搜尋結果會標出是哪一個子網路的紀錄**。重疊網段下同一個位址本來就會有多筆，但兩列長得一模一樣 —— 而其中一筆顯示上線、100% 可用，另一筆顯示離線、0%，因為一個子網路有開掃描、另一個沒有。
- **主機名稱來源的標籤不再提供刪除**。那些是「各來源分別回報了什麼」的觀測記錄，實際採用哪一個由主機名稱優先序決定。給一個 X 會讓人以為要在那裡挑，而且刪掉之後下次同步又會回來。

### 新增
- **DHCP 固定分配已納入同步並顯示** —— 這個位址是不是被綁給特定網卡，而不是動態發放。**我們支援的每一種 DHCP 來源都做了**：OPNsense（Kea 的 reservation **與** config.xml 裡 ISC 的 static mapping —— 實機上兩台防火牆剛好各用一種，所以兩條路都得走）、pfSense 的 static mappings、Windows DHCP 的保留位址、FortiGate 的 `reserved-address`。IP 詳細資料頁以「固定分配」標籤顯示，附上綁定的網卡與來源 DHCP 伺服器；IP 清單則以 icon 呈現。沒有 IP 的項目會略過：那種靜態對映只是「認得這張網卡」，並沒有保留任何位址。

  這件事之所以重要，是因為 0.5.141 修掉的那個張冠李戴 —— 一台筆電的 OS 被算到 VM 頭上：出問題的那個位址是**動態**的，才會被回收給別台機器。有固定分配的位址不會被回收，所以「這個位址是不是綁死的」正是資料看起來掛錯機器時最該先確認的一件事。

## [0.5.141] — 2026-08-04

### 新增
- **對外曝險偵測**，在異常偵測底下新增一類：哪些內部主機從外面連得到，而它的狀態是否說得過去。四條規則 —— 對外開放但完全沒有監控涵蓋、已離線卻還對外開放、所在子網路已歸檔卻還開著、DNS 還指著一台已離線的主機。**只讀已經同步進 jt-ipam 的資料**（NAT 規則、防火牆規則、DNS 記錄），偵測過程不會連線到防火牆或任何設備。放在異常偵測而不是 AI 巡檢是刻意的：這些是算得出來的事實，可以直接講，不必加「可能」。AI 對話與 MCP 也能透過 `list_anomalies` 問到。

### 修正
- **一台 macOS 的身分被貼到了 Linux VM 上**。某個 IP 顯示 OS 是 macOS（來源 Wazuh），但它的 MAC 是 Proxmox 的、PVE 裡也沒有任何 macOS 的 VM。追下去：Wazuh 的 agent `laptop-a1.local`（一台筆電，狀態 disconnected）身上還記著幾個月前拿到的那個 DHCP 位址，而那個位址早就被回收給一台 VM 用了。我們只用 IP 比對 agent，過期的登記就這樣勝出。現在的判準是：**agent 已失聯、而這個位址在它失聯之後還被偵測到活著 → 不採用它的資料**；只是關機的機器（沒有更新的存活證據）仍然保留原本的資訊。對外曝險的「有監控涵蓋」也照同一個判準 —— 失聯的 agent 沒有在看任何東西。
- **AI 對話把識別字中間的底線渲染成斜體**。`recent_ip_changes` 顯示成 recent*ip*changes。CommonMark 明訂字中間的底線不算強調，就是為了 snake_case 這種名稱；我們自己那支極簡渲染器少了這個條件，連行內程式碼裡的識別字也一起被拆掉。

### 修正
- **OPNsense 的 NAT 規則全部被記成停用**。config.xml 的解析原本只判斷 `<disabled>` 這個元素**存不存在**，但這台防火牆是明寫值的 —— `<disabled>0</disabled>` 代表啟用，只看存在與否就會讀成停用。實機上 44 筆 NAT 全被標成停用，修好後是 28 筆啟用、16 筆確實停用。解析改成兩種寫法都吃（舊版只有元素、新版明寫 0/1），JSON API 那條路徑的同類問題（`bool("0")` 會是 `True`）也一併修掉。這也正是對外曝險偵測一開始查不到東西的原因：錯的是它讀到的資料，不是規則本身。
- **三個元件在畫面上用了卻沒有 import**，執行時會安靜地消失，它的插槽內容原地變成一段放錯位置的純文字：編輯機房／地點時的單位下拉、子網路詳細資料頁的 IP 篩選框、VLAN 頁的成員子網路標籤。CI 新增一項檢查，掃描每個 `.vue` 是否用了沒 import 的 `<n-…>` 元件 —— 這類問題型別檢查、lint、build 全都會過。

## [0.5.140] — 2026-08-04

### 新增
- **AI 巡檢的發現可以一次全部清除**，讓下一次巡檢從空白開始。這是**刪除**，刻意不是「全部忽略」：忽略會留下指紋，往後每一次巡檢遇到同一件事都會自動略過 —— 用忽略實作清除，等於把你正想重新檢視的東西永久埋掉。已忽略的也一併刪除，因為那些記錄正是「不要再報這件事」的依據，留著就等於沒清。確認視窗會把後果講明白，操作也會寫入稽核記錄。
- **功能地圖頁補上 AI 巡檢**（docs/features.html）。首頁本來就有介紹，功能地圖漏了。

### 變更
- **AI 巡檢的統計數字改用全站統一的大小**（24px）。原本巡檢頁是 20px、儀表板區塊是 22px，跟正上方的 KPI 卡片並排時看起來小一號。

## [0.5.139] — 2026-08-04

### 新增
- **AI 巡檢的發現可以依分類篩選**（對外暴露／疑似遺留／資料衝突／命名不符／監測涵蓋／政策落差／其他），就放在原本的嚴重度篩選旁邊。每則發現本來就掛著這個標籤，看得到卻不能篩，等於要從一整頁高嚴重度的發現裡用眼睛挑出「所有暴露的管理介面」。

## [0.5.138] — 2026-08-04

### 修正
- **裝置清單永遠只顯示前 200 台，而且搜尋框搜不到其餘的**。某站台有 272 台裝置，儀表板顯示 272、裝置頁卻是「共 200 筆」。清單只抓了一頁，頁面上的搜尋又只在「已經載入的那幾筆」裡過濾 —— 於是新增的裝置只要名稱排序落在前 200 名之外，就會**清單看不到、用名字也搜不到**，但從機櫃點進去卻正常（那是依機櫃查詢，結果集很小）。客戶回報的原話是「新增設備都無法顯示出來，打名字搜尋也是一樣」。現在清單會分頁抓完整份，搜尋改由伺服器端執行（名稱／型號／序號／說明，不分大小寫），搜得到尚未載入的裝置。超過 5,000 台時會明講「顯示 N／共 M 筆」，不會安靜地截斷。

### 新增
- **AI 對話與 MCP 可以問異常偵測了**（`list_anomalies`）：IP 衝突、MAC 變動、失聯 IP、未授權 IP、非法 DHCP 伺服器。AI 巡檢的發現原本就問得到（`list_ai_findings`）。兩者刻意用不同方式回覆 —— 異常偵測是量到的事實，可以直接講；AI 巡檢是模型的推測，回覆時會標明並附上依據資料。這支查詢是唯讀的，且明確不會像排程掃描那樣發出通知。

### 資安
- **AI 巡檢的發現，非管理員可以透過 AI 對話問到**。0.5.137 已把巡檢的 REST 端點整支收成限管理員，但 MCP 工具還留在低一級的「全域讀取」：有萬用讀取權限的唯讀檢視者，選單裡進不去巡檢頁，卻能在對話裡把結論問出來 —— 同一份資料兩道門、兩把不一樣的鎖。巡檢與新增的異常偵測工具現在都限管理員；原本就會擋「未歸類工具」的那條測試也認得新的管理員層級，之後新增工具一樣不選層級就過不了。

### 變更
- **可用率追蹤清單的排序按鈕補上 icon**，與全站其他按鈕一致 —— 那個對話框裡就只剩這幾顆是純文字。
- **裝置詳細資料頁可以刪除裝置**。原本刪除只在清單上 —— 而找不到的裝置，恰好也是在清單上刪不掉的那些。

## [0.5.137] — 2026-08-03

### 變更
- **AI 巡檢整支限管理員**。功能移進管理區之後，權限就要跟位置對得起來 —— 原本「看發現」只需要全域讀取權限，會出現「選單看不到、直接打網址卻進得去」，那是最糟的一種：看起來有管控，實際上沒有。前端路由與儀表板區塊也一併收斂（非管理員看到區塊只會拿到 403，等於放一塊壞掉的畫面）。理由不只是位置：巡檢結論本身就是一份跨部門的弱點清單（哪些網段沒監測、哪些管理介面放在一般網段），不該給只被指派特定物件的帳號看。
- **AI 巡檢清單加上可排序的欄位標題**（嚴重度／狀況／日期時間／動作）。發現本身是長文，做成表格反而難讀，所以只把「可以排序的那幾件事」做成標題列。預設嚴重度由高到低；同嚴重度時以時間為第二順位，順序才不會每次重整就跳動。

## [0.5.136] — 2026-08-03

### 修正
- **`ping` 工具在「非特權 ICMP socket 開得起來」的機器上直接 500**。uvloop 沒有實作 `loop.sock_sendto` / `sock_recv`，那條路一呼叫就丟 `NotImplementedError`。諷刺的是**開不起來**的機器反而正常（會退回外部 `ping`）—— 也就是說，照著我們自己給的排除說明去放寬 `net.ipv4.ping_group_range` 之後，才會踩到這個錯。已加退路，實測本機 ping 127.0.0.1 與閘道都正常回應。

### 變更
- **異常偵測可以指定要看哪些子網路**（異常偵測頁的「偵測範圍」，或子網路編輯頁的「納入異常偵測」—— 兩處寫的是同一個欄位）。訪客、實驗、外包商網段本來就一直有異常，關掉才不會把真正該處理的埋掉。
- **未授權 IP 不再被 169.254.x.x 灌爆**。那是機器 DHCP 拿不到位址時自己指派的 link-local，是「沒拿到 IP」的徵狀，不是有人偷接東西 —— 一台正式站台的清單 53 筆全是這個，真正該看的完全被埋掉。同時排除多點傳送／保留位址，以及網段的網路位址與廣播位址（那兩個不對應到任何一台機器），還有落在所有子網路之外的位址（本來就不是這套 IPAM 在管的）。
- **AI 巡檢移到「管理」區，排在異常偵測後面**。兩者都是「找問題」，但**刻意不合併**：異常偵測講的是量到的事實（ARP 就是看到兩個 MAC），AI 巡檢是模型的推測、可能講錯。混成一份清單會分不出哪些結論可以直接相信。
- **ping 送不出封包時，旁邊多一個「怎麼修？」**，點開直接給兩種可照做的設定（放寬 `ping_group_range`／給服務 `CAP_NET_RAW`），並說明兩者的權限差別。原本只說了「壞了」，沒說要做什麼。
- **連線診斷獨立成自己的頁籤**。它會真的從伺服器送封包、有次數上限、會寫稽核記錄，跟同頁上方那些純計算工具性質完全不同，混在一起容易讓人以為那也只是算一算。
- **AI 巡檢：高／中的發現左邊加一條色條**，一眼看得出哪幾筆要先處理（低不加 —— 每列都有色條等於沒有重點）。統計數字改為有框線的卡片，跟儀表板其他 KPI 一致。
- **AI 巡檢的說明文字不再提前折行**。原本設了 78ch 的行寬上限，但中文一個字約等於兩個 `ch`，換算下來只有 39 個字，寬螢幕上右邊空一大片、每兩三個字就折一次。
- **可用性追蹤可以設定排序**（編輯追蹤清單裡）：依 IP／主機名稱／可用率，遞增或遞減。沒有資料的一律排最後 —— 那不是「可用率 0」，是「不知道」。

## [0.5.135] — 2026-08-03

### 新增
- **非法 DHCP 伺服器偵測**（掃描代理）。代理在網段上廣播一次標準的 DHCPDISCOVER，把所有回應的主機記下來；**任何在發 IP、卻沒有被標記為 DHCP 伺服器的主機**，都會在「異常偵測」頁最上方以紅色警示列出（含 IP、子網路、MAC、廠商、它發出的位址與指定的閘道）。這是少數「一出現幾乎必定有事」的異常 —— 常見來源是有人接了家用路由器，或某台虛擬機忘了關 DHCP，結果把錯的位址與閘道發給整個網段。
  - **預設關閉**：它會在網段上廣播，該不該做由管理員在各子網路的掃描設定裡逐個決定。
  - 只送 DISCOVER、不送 REQUEST —— 不會真的拿走一個位址。
  - 「合法與否」是**查詢時**才比對的，不是寫死在觀測記錄裡：事後把某台標記為合法，舊記錄會跟著變成合法，而不是留下一筆永遠錯誤的「非法」。
  - 比對帶著子網路一起看 —— 重疊網段（多個單位共用 192.168.1.0/24）下，別的網段的標記不會被誤用成這個網段的授權。
  - 經由 DHCP relay 轉送的回應不算非法：那台伺服器本來就不在這個網段上。
- **子網路可以逐個決定要不要納入 AI 巡檢**（子網路編輯頁的勾選框，或 管理 → LLM / AI 的多選一次挑）。兩處寫的是**同一個欄位**，改哪邊都一樣。敏感網段可以完全排除，不送給模型。
- **AI 巡檢** —— 讓語言模型定期檢視這套系統管理的 IPAM 資料，找出可疑、不合理或有資安疑慮的地方（久未出現卻仍登記在用的位址、名稱與實際用途對不上的主機、重複或互相矛盾的登記、完全沒有監測來源的網段…）。三個入口：左側新增「AI 巡檢」頁、儀表板新增摘要區塊、管理 → LLM / AI 可設定是否啟用排程與執行時刻（預設關閉）。排程跟著既有的同步排程跑，不另建工作；頁面上另有「**立即執行**」按鈕，不必等排程。
  - **取樣先過權限**：巡檢只看得到執行身分看得到的資料，不是把整個資料庫倒給模型。
  - **每一筆發現都附依據資料**，IP 可以直接點過去查證。沒有依據，那些話就只是無從查核的斷言。
  - **模型輸出一律當成不可信輸入**：自創的嚴重度與分類會被降級、長度截斷、解析不出來就整批不存。
  - 「這次沒發現問題」與「模型壞掉了」分開處理 —— 混為一談的話，不是把故障當成平安，就是把平安報成故障。
  - 發現內容用**執行者的介面語言**撰寫（繁中／英文）。
  - **巡檢用哪個模型可以另外指定**（管理 → LLM / AI）。留空＝沿用 AI 對話的模型。巡檢是一次送進大量資料的批次工作，跟互動對話的取捨不一樣 —— 可以指定較大的模型換判斷品質，或較小的省算力。
  - **排程設「每天幾點幾分」，可以設多個時刻**（不是「每 N 小時」）。間隔式排程會跟著每次的執行時間往後漂，跑幾天之後就沒人說得準它是在半夜還是上班時間打 LLM。時刻以伺服器時區為準，設定畫面會直接寫出目前是哪個時區。
  - **資料依模型的上下文長度自動切批送**，執行時顯示目前跑到第幾批、共幾筆 IP、用哪個模型、已找到幾筆，並附進度條與已耗時。
  - **改成背景作業**：按下「立即執行」馬上返回，巡檢在背景跑。可以關掉分頁、切到別頁、重新整理 —— 回來還看得到進度與結果。原本綁在連線上，一離開頁面整個作業就被取消，跑了十幾分鐘的東西一點都拿不回來。
  - 已經有一次在跑時再按會被擋下（回 409），不會排隊 —— 兩次同時跑不是變快，是互相拖死同一台 LLM。
  - **限制單批產出長度**：實測看過模型卡在重複輸出的迴圈裡，一批寫到 54,000 字還沒停，把整個逾時燒完才失敗。
  - **忽略是「往後都忽略」，而且可以反悔。** 判定為誤報按下忽略後，之後的巡檢再找到同一件事會自動歸入已忽略，不會每天跳回來（比對的是「分類＋依據資料的 IP 清單」的指紋，不是標題 —— 標題每次都是模型重寫的，用標題比對等於幾乎比不中）。誤按了就切到「已忽略」按「取消忽略」放回未處理。
  - **關掉模型的思考模式**：思考過程也算在產出額度裡，實測 gemma4 一批寫了 10,401 字的思考，真正的答案因此被切斷、三批全部解析失敗 —— 一筆發現都沒存下來。關掉之後同一份資料順利跑出 5 筆發現。舊版 Ollama 不認這個參數時會自動退回重送。
  - **巡檢的上下文長度（num_ctx）可以另外設定**，留空＝沿用對話模型。這個值決定一批塞得下多少筆資料：開大批次變少、整體更快，代價是記憶體／VRAM。
  - **被切斷的 JSON 會撿回已經寫完的那幾筆發現**，而不是整批丟掉。只有連一筆完整的都撿不到才算失敗。
  - 排程的「上次執行時間」獨立記錄，不是拿最後一筆發現回推 —— 否則一次乾淨的巡檢什麼都不會寫，排程就會判成從沒跑過，每一輪同步（約 5 分鐘）再打一次 LLM。
- **AI 對話與 MCP 補上近期新功能**：`list_firewalls` 改為一次涵蓋 OPNsense / pfSense / FortiGate（只回其中一種的話，模型會拿半份清單當成全部去回答）；新增 `list_dhcp_ranges`（各整合同步回來的 DHCP 發放範圍）、`list_fortigate_policies`、`list_fortigate_addresses`、`list_ai_findings`。系統提示詞也補上憑證集中保管與派送、DHCP 範圍與巡檢發現的說明。

### 修正

- **BIND 9 整合完全無法運作**（客戶回報，v0.5.129 起）：接上去、狀態顯示啟用中、同步也不報錯，但 DNS 記錄永遠是 0 筆。兩件「存了卻沒生效」的事：
  - **zone 清單沒有輸入欄位**。DNS 協定沒有「列出所有 zone」的方法，同步只認明列的 zone —— 而畫面上根本沒得填，於是一次 AXFR 都不會發生。現在設定頁多了「Zone 清單」（含反解 zone），沒填時同步會**明確報錯**，而不是安靜地跑出 0 筆。
  - **TSIG 金鑰沒有被拆解**。畫面提示輸入 `algorithm:keyname:base64key`，後端卻把整串當成密鑰、金鑰名稱另外去別處讀（那裡從來沒被寫入）→ 金鑰名稱永遠是空的 → 等於沒設 TSIG → BIND 拒絕 AXFR。現在會照提示的格式拆開。
  - 順帶修掉：BIND9 的設定原本只在「有使用者名稱或有 TLS 驗證選項」時才會被寫進 extra_config —— 這兩個 BIND9 都沒有，所以連 zone 清單都會被丟掉。

- **一次把整份清單送給模型 → 超出上下文被截斷 → 模型回一篇散文，畫面上卻像「跑完了，沒發現問題」。** 正式環境 360 筆 IP 約 75,000 字元，遠超過 `num_ctx=16384`；Ollama 會安靜地把提示詞前段截掉，模型收到半截輸入，於是寫了一篇「網路環境總覽」回來。三處一起修：資料依上下文長度**切批**送、對 Ollama 指定 `format=json` 強制結構化輸出、估算 token 時**中日韓字元以 1 字 1 token 計**（用「4 字元 1 token」估中文會嚴重低估）。
- **執行失敗在畫面上看不出來。** 錯誤只用一閃即逝的訊息提示，而「最後執行」時間照樣更新 —— 兩者加起來看起來就像成功。改成失敗時在頁面上留一則不會自己消失的錯誤，並附模型實際回了什麼。
- **時間欄位型別不合造成的 500**：model 宣告成 naive `DateTime`、資料庫欄位卻是 `timestamptz`，讀取沒事、寫入帶時區的值才炸。影響「忽略巡檢發現」（整個功能不能用）與電路的開通日 / 合約到期日（用 API 帶時區時間會 500）。同時加了一條測試掃過所有 model 的時間欄位，避免下次再犯。

## [0.5.133] — 2026-08-02

### 新增
- **工具 → IP 位址再加三項診斷**，全都不需要任何特權：
  - **TLS 憑證檢查** —— 顯示對方實際送出的憑證：主體、簽發者、效期（剩餘天數依急迫度標色）、SAN、協商的版本與加密套件。它**刻意不先驗證就取回** —— 自簽、過期、名稱不符的憑證正是最需要看到的時候，因為驗不過就拒絕顯示反而失去意義。是否通過系統信任庫、名稱是否相符、是否自簽都各自獨立成欄，而不是全部塌成一句「失敗」。
  - **HTTP 檢查** —— 狀態碼、完整轉址過程，以及值得一看的標頭（Server、Content-Type、HSTS）。
  - **批次反向 DNS** —— 一次看出網段裡哪些位址有 PTR、哪些沒有，並給出計數。逐台查太瑣碎，瑣碎到大家乾脆不查。
- **裝置詳細資料頁現在會顯示它是不是 DHCP 伺服器**，並以提示說明是名下哪一個位址帶這個角色。這個旗標本來就存在、IP 清單也早就在顯示，唯獨看裝置時毫無線索 —— 同一台主機在不同頁面講的話不一樣。
- 同一組角色標籤（閘道、DHCP 伺服器、在 DHCP 範圍內）也補到 IP 詳細資料頁 —— 它原本顯示得比「點進來之前那張清單」還少。

### 修正
- **連線診斷不再一半兩欄一半一欄。** 每一項都會產生結果表格，半欄寬度根本不夠 —— TCP 卡片把 `Connection refused` 折成了 `Connectio n refused`。現在一律整列，儲存格也不再從字中間斷行。上方的計算工具維持兩欄：那是精簡的鍵值小工具，**一致地不同**比勉強對齊更好讀。
...
### 新增
- **TWNIC 匯入實作完成** —— 原本只是一行「規劃中」。兩個註冊管理機構現在都改為透過 RDAP 線上查詢：RIPE 直連，TWNIC 走 APNIC（台灣網段會被自動轉到 TWNIC 自己的資料庫 —— APNIC 是台灣的權威來源，TWNIC 是其下的國家級註冊管理機構）。寫入前會先顯示登記資料：網段名稱、國別、位址範圍、配發類型、聯絡人、備註，以及這份資料是從哪個網址查來的。
  - **RDAP 查不到「handle → 網段」**：APNIC 的 entity 查詢直接回 404，RIPE 就算回 200 內容也不含網段。原本 UI 上的 Handle 欄位承諾了一件協定給不出來的事，已移除；要用 handle 或機構名稱查，改為貼上 whois 輸出（沿用既有解析器）。
  - **RIPE 分頁其實也是壞的**：前端送 JSON、後端要檔案上傳，一律回 422。那兩個欄位從來沒接上任何東西。
- **工具 → IP 位址新增連線診斷**：ping（可一次多個目標並設併發數）、路徑追蹤、TCP 埠連通性。目標可填 IP、主機名稱，或直接給網段（會展開成各個主機）。
  - **完全不經過 shell** —— 指令以參數陣列執行，目標另外驗證為位址或主機名稱當第二道防線。目標數、併發數、單目標逾時與整體期限都有上限，並對每個使用者限流、每次執行寫入稽核記錄，避免伺服器被當成掃描跳板。
  - 路徑追蹤優先用 `tracepath`：不需特權，而且會回報 `traceroute` 沒有的路徑 MTU。**沒有回應的躍點會列出而不是略過** —— 藏起來的話路徑看起來像 1→3→5 中間沒東西。時間不夠時會回傳已經探到的部分，而不是整個丟掉。
  - TCP 埠測試往往比 ping 有用：擋 ICMP 的主機 ping 不通，但你真正要用的那個埠是通的。

### 修正
- **IP 衝突清單一個 MAC 位址都沒顯示。** 渲染邏輯用「這個陣列的元素有沒有 `last_seen_at`」判斷它是不是位置資訊 —— 而 MAC 項目也有這個欄位，於是被拿去畫「裝置／埠」欄，那個物件兩者皆無，結果整張表都是「—」，而報告存在的唯一理由（哪幾個 MAC）反而沒畫出來。
- **衝突現在看得懂了。** 每個 MAC 會帶出 OUI 廠商，並標示本地管理位址。實機資料上 133 個衝突 MAC 有 64 個是本地管理（虛擬機、容器、手機 MAC 隨機化），所以「真實 MAC ＋ 隨機 MAC」多半是同一台裝置換了位址，不是兩台在搶。四個異常類別也都加上「這是什麼、為什麼會出現」的說明。（廠商查詢一開始還寫錯：`vendor_map()` 的 key 是正規化後的 6 碼前綴而非完整 MAC，導致每一筆廠商都是空的 —— 不會報錯，只會靜默顯示未知。）
- `detect_ip_conflicts` 直接回傳 asyncpg 對 INET/MACADDR 欄位產生的物件而非字串（已知地雷 #10）。

## [0.5.130] — 2026-08-01

### 變更
- **Windows 憑證派送的支援範圍明訂為 Windows Server 2019 以上**，不再支援 Server 2016。交給代理的 PKCS#12 仍維持 PBESv1-SHA1-3DES，但理由換了：它是 Windows CryptoAPI 全版本都吃的形式，而這裡的加密其實保護不到任何攻擊者搆得著的東西 —— 這個 PFX 是每次請求即時產生、用只存在記憶體的隨機密碼加密、匯入後立刻丟棄，**從不落地**。拿一條確定可用的路徑去換一個保護不到東西的演算法強度並不划算。（已在實機確認：現行版本的 Windows 兩種加密都匯得進去，所以這是相容性下限而不是限制。）

### 修正
- **儀表板每張卡片的標題都補上 icon，而且 icon、標題、計數標籤現在對齊了。** 原本只有可用性追蹤那張有 icon，看起來像外掛上去的而不是頁面的一部分。沒對齊是因為那個標題用了會把每個子項各自包起來的間距元件，18px 的 icon、一行文字、22px 的標籤三者各自貼著自己的基線。現在卡片標題統一走一個共用的小元件、由**單一條 flex 規則**決定對齊，而不是每張卡片各排各的 —— 十張實測偏移 0.01px。
...
### 修正
以下都是把新的 Windows 代理放到**實體 Windows 11 + IIS 主機**上跑才發現的，紙上看不出來 —— 其中兩個是「宣稱成功但其實什麼都沒做」。

- **同一張憑證的第二個部署會被靜默跳過，還回報成功。** 部署狀態只用「憑證＋profile」當鍵，所以一台主機上同一張憑證用在兩個繫結（例如 443 上兩個 SNI 站台）時，只有第一個會被更新，第二個永遠被當成「已是最新」—— 續簽後默默過期，回報卻是 ok。現在狀態鍵另外包含「這個部署與別人不同之處」（`iis` 是繫結、`files` 是輸出路徑）。**已上線的 bash 代理有一模一樣的缺陷**（手動模式把同一張憑證寫到多個路徑），一併修正（代理 0.4.174）。舊版寫下的狀態仍然採用，但只限沒有可區分目標的部署。
- **一台主機有多個 IIS 站台時，SNI 繫結從來沒有被裝上憑證，卻回報成功。** 判斷「要的憑證是不是已經綁好了」原本是靠開一條 TLS 連線來看，但**還沒註冊憑證的 SNI 繫結，一樣會被同一個埠上的非 SNI 萬用繫結回應**。當那張後備憑證剛好就是要部署的那一張，代理就判定已經綁好、回報 ok、狀態記成完成 —— 而 `netsh http show sslcert` 上那個主機名稱其實一筆註冊都沒有。現在改成直接問 http.sys 這個**特定繫結**的狀態，且比對的是 40 位十六進位的指紋值，不看 netsh 那些會被在地化的標籤。
- **Windows 安裝程式根本註冊不了排程工作。** `schtasks /TR` 把整串指令當成單一引數，它的引號規則會弄壞任何含空格的路徑 —— 而預設安裝路徑 `C:\Program Files` 必定含空格。改用 `Register-ScheduledTask`（引數字串原樣傳遞）。執行身分改用 SID 指定而非名稱 `SYSTEM`（名稱是在地化的）。
- IIS 部署失敗且原本沒有已繫結憑證時，會在沒有站台回應的連接埠留下 http.sys 註冊；現在會收回，失敗不留痕跡。
- 設定檔缺漏或不存在時會噴出整段 PowerShell 堆疊追蹤把真正的訊息埋掉；現在只印一行可讀訊息並以 2 結束。
...
### 修正
- **儀表板可用性追蹤：在挑選視窗一打字搜尋，已加入的 IP 就變成一長串 UUID。** 搜尋會把整份選項清單換成搜尋結果，已選那筆的選項因此消失 —— 沒有選項可對照，下拉元件只好把原始值（UUID）直接畫出來。現在已選項目會用本地快取保住顯示文字，並一律併回選項清單。若某個 IP 已無法存取，會明確標示而不是丟出 UUID。

## [0.5.125] — 2026-07-31

### 新增
- **儀表板的可用性追蹤區塊** —— 佔整列全寬，可自行挑選要追蹤的 IP（最多 30 個），把它們的 90 天長條上下對齊排在一起，各自標示正常運作百分比，點列可直接跳到該 IP。選擇的清單存在既有的通用 `user_preferences.pinned` 對照表裡（跟著帳號走、換瀏覽器也保留），**不需要改資料庫結構**。
  - 背後是新的 `POST /api/v1/addresses/uptime/batch`，不論要幾個 IP 都只打兩次 DB —— 逐一呼叫單一 IP 的端點 30 次會變成 60 趟往返。它回的是**每個 IP 各一條**（與裝置端點不同，那支是把整台裝置合併成一條），保留你排的順序，而且**看不見的 IP 會直接略過而不是報錯**，權限變動時區塊不會整個壞掉。
  - 誠實原則與詳細資料頁的長條一致：沒有上線偵測來源的 IP 整條灰，百分比顯示「—」而不是 0% 或 100%。

## [0.5.124] — 2026-07-31

### 修正
- **PVE 帳號啟用兩階段驗證後無法連主控台**（GitHub issue #23，由 @kelp45705753-bit 回報）。Proxmox 對啟用 TFA 的帳號，`/access/ticket` 回的是 **HTTP 200** 加一張**挑戰票證** —— `{"ticket": "PVE:!tfa!…", "NeedTFA": 1}` —— 而不是錯誤。程式把它當成正常 ticket 收下，於是失敗要等到開 websocket 時才浮現，而且訊息完全看不出真正原因。現在登入會辨識出挑戰票證，並用 `tfa-challenge` 加 `password=totp:<驗證碼>` 換到真正的 ticket；沒帶驗證碼時回一個專屬的 `tfa_required`，讓主控台跳出 6 位數輸入欄，而不是掉進一個看不懂的錯誤畫面。驗證碼錯誤或逾時也會照實說明，不會再拿著挑戰票證去連線。沒有啟用 TFA 的帳號維持單次請求，不受影響。

### 變更
- 用詞：程式註解與繁中變更記錄中的「詳情」改為「詳細資料」、「膠囊」改為較平實的說法（台灣慣用）。

### 附註
- 兩階段驗證的換票流程依 Proxmox 官方文件實作，但**沒有啟用 TFA 的實機 PVE 帳號可以驗證**；單元測試涵蓋了挑戰票證、成功換票、驗證碼錯誤，以及未啟用 TFA 的原有路徑不受影響。

## [0.5.123] — 2026-07-31

### 修正
- **從未中斷過的 IP 被畫成「未監測」。** 長條原本純粹由 `effective_status` 的轉換記錄重建，但一個從加入以來就沒斷過的 IP **根本不會產生任何轉換** —— 於是整條灰，即使它上方的欄位明明寫著「上線、30 秒前剛出現」。**「沒有轉換」不等於「沒在監測」**。重建現在會一併看 IP 目前的狀態與 `last_seen_*`：有存活來源、且視窗內沒有任何轉換時，用目前狀態從「IP 加入時間」起回填（更早之前仍然是未知）。兩個真實的線上 IP 從 90 天全灰變成 67 天全綠、100%。
- **持續一個月的離線看起來像一個月的零星中斷。** 原本只要當天有中斷就一律塗橘，所以一個 7 月初就離線的 IP 會變成 29 格一模一樣的橘色。現在把每日狀態分開：**橘＝當天有斷也有通**（真正的中斷時段）、**紅＝整天都不通**。同一個線上 IP 現在顯示 29 格紅、2 格橘，這才是實際發生的事。

### 變更
- 長條改為直角（不再是兩端半圓的圓角），滑鼠移上去變成手指游標（每一格都有 tooltip）。

## [0.5.122] — 2026-07-31

### 新增
- **IP 詳細資料頁與裝置詳細資料頁的存活狀況長條圖** —— 90 天 status page 式長條，綠＝正常、橘＝當天曾中斷、灰＝無資料。裝置的長條會合併它名下所有 IP：**任一 IP 當天中斷就標中斷**，所以單一介面掛掉仍然看得見。
  - 資料庫裡沒有 per-IP 的時序取樣，所以每日狀態是從 `ip_change_log` 既有的 `effective_status` 轉換記錄**重建**出來的：某個狀態會延續到下一筆轉換，第一筆轉換之前一律未知。
  - **沒有資料的日子是灰的，絕不塗綠。** 沒有存活來源（掃描代理或 LibreNMS）的 IP 會是整條灰 —— 那才是誠實的訊號：它代表「沒在監測」，不是「一切正常」。滑鼠移上去會說明原因。
  - **正常運作百分比只計算有資料的天數。** 一個只監測了三天、全部正常的 IP 顯示 100%，而不是被 87 天灰色稀釋、也不是把灰色當成中斷來扣分。分母就寫在數字旁邊，避免被斷章取義。
  - 灰色走 Naive UI 主題變數而非寫死色碼，深色模式下才不會刺眼；綠與橘維持固定色，因為那是狀態語意色，在兩種主題下都讀得對。

## [0.5.121] — 2026-07-31

### 修正
- **FortiGate 的 VPN 同步分不出「沒人連線」與「端點讀不到」。** 兩者都會得到 `ssl_sessions: 0`，因為端點失敗被 `except FortiGateError: continue` 吞掉了。客戶的實機同步回報的正是這個數字，而從稽核摘要完全看不出 SSL-VPN 的解析到底有沒有在運作。現在所有 VDOM 的端點都失敗時，摘要會多帶 `ssl_unavailable`／`ipsec_unavailable`，讓「真的是 0」與「默默失敗」長得不一樣 —— 對一個沒有實機就開發出來的整合，這個區分最要緊。

### 附註
- 感謝客戶把所有同步開關都打開，**FortiGate 現在已對實機驗證過**：VDOM 探索、ARP（454）、DHCP 租約（339）、DHCP 發放範圍（3）、位址物件（632）、防火牆政策（211）、NAT（14）、IPsec 通道（4）。SSL-VPN 連線回報 0；有了上面那個改動，下一次同步就能說清楚那是「沒人連線」還是「端點讀不到」。

## [0.5.120] — 2026-07-31

### 修正
- **稽核記錄的「目標」欄對所有整合都只顯示截斷 UUID。** 客戶實測 FortiGate 時發現那一欄是 `a1b2c3d4…` 而不是實例名稱 —— 同型整合有多台時，稽核根本看不出是哪一台在同步。`_LABEL_REGISTRY` 只涵蓋 14 種物件型別，所有整合實例、代理、憑證與 API 權杖都掉進「直接顯示 id」的預設分支。已補上 26 種（每一條都驗證過真的指得到對應 model 與欄位），整合列現在也能點進它的設定頁而不只是純文字。並加測試釘住「每條登錄都解析得到」與「整合型別不得缺漏」—— 新增整合卻忘了登錄的話，會無聲地退回顯示 UUID。
...
- **近期新增功能的 RBAC 稽核 —— 修好 6 個真漏洞。** 下面每一條都先對照程式碼驗證過才動手。
  ...
  - **地點的 IDOR**：`GET /locations/{id}` 與 `GET /locations/{id}/floorplan` 只要求**認證**，所以任何已登入帳號憑 id 就能讀任何地點、下載任何機房平面圖。兩支現在都要 `require_object_perm("location", "read")`。系統性掃過所有逐物件詳細資料端點，確認缺口只有這兩支。
...
### 修正
- **時間顯示成 UTC 而非本地時間** —— 作業表格（排入／完成時間）、子網路詳細資料與裝置詳細資料的最後上線欄、以及異常偵測明細，原本只把 ISO 的 `T` 去掉、沒轉時區，所以顯示 UTC。改用全站共用的本地時間格式（跟隨觀看者瀏覽器時區），與其他頁面一致。
...
### 變更
- **連線清單 OS 欄改成與 IP 詳細資料頁一致** —— 共用 `OsCell`：OS 圖示 + 在地化家族名 +（來源）標註，滑鼠移上顯示原始偵測字串；顯示的 OS 是依來源順序算出的有效值（與 IP 詳細資料相同），不再只是掃描代理原始猜測。
...
### 變更
- **連線按鈕改為單純按鈕** —— 移除分割按鈕右側的下拉箭頭（「另開視窗」選單），SSH／RDP／VNC／noVNC／BMC 在連線清單頁與 IP 詳細資料卡片都改成點一下就開主控台（新分頁）。連線清單列高收緊。BMC 空白提示精簡成一行（細節收進「看設定教學」）。


## [0.5.69] — 2026-07-01

### 變更
- **連線按鈕 —— RDP/VNC/noVNC 圖示更好辨識** —— 三者原本共用「螢幕 + 10px 小字母」很難分辨；字母改為大（13.5px）且粗、佔滿螢幕，R / V / N 一眼可辨。分割按鈕的下拉箭頭收窄（連線清單頁 + IP 詳細資料頁）。
...
- **BMC 主控台畫面改成與 SSH/RDP/VNC 一致**（卡片高度、左側標籤表單、「記住帳密」用 switch、標題 icon 對齊、狀態標籤工具列、滿版終端機）＋ 空白畫面時的「按 Enter」提示（SOL 閒置正常）。
...
### 修正
- **IP 申請清單真正顯示子網路 CIDR。** 0.5.56 讓前端改用 `subnet_cidr`，但清單端點從未填這欄（只有詳細資料端點有填），
...
- **異動記錄超過指定天數的項目以淡色顯示**，讓近期變更更明顯。天數於 管理 → 系統設定 → 顯示 設定
  （預設 30 天；0 = 不淡化）。套用在 IP 詳細資料的異動時間軸與「IP 異動記錄」頁。
...
### 修正
- **連線管理頁的 PVE 主控台按鈕對齊 IP 詳細資料頁** —— 標籤只放 noVNC／xterm，右上角加一個「PVE」小標
  （取代原本內嵌的「·PVE」）。
...
- **連線管理頁的 PVE 主控台按鈕對齊 IP 詳細資料頁**：依類型顯示 xterm（CT）／noVNC（VM）、改用醒目色（橘／PVE），
...
### 修正
- **裝置詳細資料頁：「編輯」改為就地開視窗**（原本會跳到裝置清單）。裝置編輯視窗抽成共用元件 `DeviceEditModal`。
...
- **掃描代理：新增「相依套件」欄。** 代理會回報它的探測工具盤點；欄位顯示裝好幾個（例如 `4/7`），點下去
  開詳細資料：每個工具裝了沒、版本多少、用於哪些探測（nmap → OS/連接埠、nmblookup → NetBIOS、avahi-resolve
...
- 主機為 Proxmox VM 客體時，關係圖會畫出它所在的 PVE 節點（及該節點的機櫃/機房）——IP 與裝置詳細資料頁皆是。
...
- **瀏覽器內 RDP 連線管理（Beta）。** 直接從 IP 詳細資料頁開 Windows RDP 桌面 —— 已對「強制 NLA 的
  ...
  - 每 IP `rdp_enabled` 開關（migration 0083）；權限 `can_use_rdp`（deny-by-default，沿用 `can_ssh`
    能力）；詳細資料頁分割按鈕 + 進階→連線管理 的「RDP」篩選/操作。
...
- **瀏覽器內 VNC 連線管理（Beta）。** 同一套模式套用於 VNC（RFB）目標 —— 已對真實 VNC 伺服器驗證。
  - 每 IP `vnc_enabled` 開關（migration 0084）；權限 `can_use_vnc`；詳細資料頁分割按鈕 + 連線管理「VNC」。
...
### 變更
- 進階→連線管理 一併列出 SSH/RDP/VNC 目標；OS 欄改用與詳細資料頁相同的來源優先序解析。
...
### 變更
- IP 詳細資料頁的「SSH 連線」改為**點主鍵開新分頁**、**下拉開新視窗**（移除頁內嵌入式終端機）。
...
- **IP 位址 SSH 連線管理（嵌入式 / 另開視窗終端機）。** 在 IP 編輯視窗可開關「啟用 SSH 連線管理」；
  啟用後，具權限者會在詳細資料頁右上（編輯鈕左側）看到「SSH 連線」分割按鈕：主鍵在本頁開啟 xterm.js 終端機，
...
### 變更
- **子網路詳細資料頁工具列加「刪除」按鈕**（帶確認框）。原本只能回「全部子網路」清單頁、用操作欄垃圾桶或批次刪除，
...
- **憑證派送代理可對應到裝置。** 編輯派送代理時可選擇「對應裝置」（`cert_agents.device_id`，migration 0080，
  裝置刪除→SET NULL）。對應後：①「派送代理」清單與「進階 → 憑證派送現況」頁的**代理名稱可點**，直接進入該裝置詳細資料；
  ②**來源 IP 欄可點**——後端把代理回報的來源 IP 解析到 IPAM 對應的位址（重疊網段時優先取掛在該對應裝置上的同 IP），
  前端連到該位址詳細資料。未對應裝置或來源 IP 在 IPAM 查無對應時，維持純文字。
...
### 變更
- **Graylog DSV 頁面改成一張可擴充的端點表格 ＋ 詳細資料抽屜。** 原本每個 DSV 來源各攤一張卡片、兩個網址框，
  防火牆一多就很亂；現在所有 DSV 端點（IP→主機名稱、每台防火牆的規則與別名查表）統一列在一張表
  （名稱／對照／狀態／操作），點「詳細資料」開抽屜顯示 HTTPS ＋ 內網 HTTP 網址、複製鈕與該來源設定
...
### 變更
- **NetBIOS / mDNS 主機名稱來源在 IP 詳細資料顯示在地化標籤**（來源標籤與「釘選主機名稱來源」下拉），與來源優先序頁
...
### 安全性
- **RBAC IDOR 修補** — 多個詳細資料／彙整端點只憑物件 id 取資料、未做物件層級可見性檢查，
...
### UI / 文件
- 修正區段詳細資料頁缺少的 i18n key（「顯示順序」原本顯示原始 key）。
...
- **掃描代理 / 裝置詳細資料**：表格欄寬收緊——操作欄不再被擠出右側、空欄不再吃滿寬度、MAC 與時間不再折行。
...
- 申請詳細資料頁顯示**關卡進度**（哪些關卡已過、目前待哪一關）；依序模式下，輪到某關才通知該關審核人。
- IP 申請清單對審核人的待審項目直接提供 **核准 / 駁回** 按鈕（不必點進詳細資料）。
- 申請詳細資料頁全面中文化；顯示子網路 CIDR（可點）與「**將配發的 IP**」——含系統自動挑的第一個
...
### 新增 — DHCP
- 子網路詳細資料在有 OPNsense DHCP 發放範圍時多顯示 **DHCP 發放範圍** 欄（無資料則不顯示），
  IP 清單並新增 **只看 DHCP** 篩選。
...
- NAT 頁：滑過已關聯到 jt-ipam 的 IP 會即時彈出該 IP 細節（主機名稱 / 狀態 / MAC / 製造商 /
  子網路 / 單位 / 裝置 / 交換器位置…），懶載入；點擊進入該 IP 詳細資料頁。

### 變更
- 新增的子網段繼承父網段的單位；`rebuild_subnet_hierarchy` 會自動修補（多層串接）。
- 左側子網路樹：下層子網段改為真正可展開的巢狀節點＋連接線（取代「↳」首碼）；父節點標題仍可點進詳細資料。
- 左側選單版本號字體放大。
...
### 變更
- **統一子網路編輯**：子網路列表與子網路詳細資料頁改共用同一個 `SubnetEditModal` 元件，
...
- **OS 來源優先順序**（掃描代理 / LibreNMS / Wazuh）：在「名稱 / ARP 來源順序」頁可拖拉
  排序，決定多個來源都回報 OS 時以誰為準；IP 詳細資料的 OS 欄位會顯示解析出的來源。
...
### 變更
- OS 偵測結果在 IP 詳細資料主要欄位表中以**「作業系統」**欄位呈現（上方），子網路 IP
  清單也新增可切換的 **OS** 欄。
...
- **全域基礎設施資料的 RBAC 收斂**：`require_global_read` / `has_global_read` /
  `can_edit`。列表、詳細資料、搜尋、儀表板彙總、計數與趨勢全部依使用者可見範圍縮放；
  操作按鈕依能力反灰。
- **纜線追蹤**（NetBox 風多跳，migration 0063）：新增 `device_ports` 表，支援
  bridge → NIC → 外部裝置穿透。
- 機櫃**半 U** 支援與正 / 背面視覺化；裝置詳細資料的機櫃圖標示本裝置位置。
...
- 裝置詳細資料：Wazuh 代理 + Proxmox VM 面板（依 IP 比對）；編輯鈕。
